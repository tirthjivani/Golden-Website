"use client";

import { useEffect, useState } from "react";

const PRELOAD_IMAGES = ["/residential-hero.jpg", "/commercial-hero.webp"];

const ICON_PATHS = [
  "M17.8629 0.00012207C18.1075 8.66038 18.2705 17.3238 18.3521 25.9903C18.4111 30.3236 18.4238 34.6568 18.4575 38.9854L18.4954 51.9806L18.4617 64.9757C18.4238 69.3089 18.4153 73.642 18.3563 77.9754C18.2719 86.6388 18.1075 95.3022 17.8629 103.966C17.6183 95.299 17.4539 86.6356 17.3695 77.9754C17.3105 73.642 17.2978 69.3089 17.2641 64.9757L17.2303 51.9806L17.2641 38.9854C17.2978 34.6568 17.3105 30.3236 17.3695 25.9903C17.4539 17.3301 17.6183 8.66667 17.8629 0.00012207Z",
  "M22.4224 21.9196C22.667 28.7534 22.8315 35.5889 22.9158 42.426C22.9749 45.8445 22.9875 49.2631 23.0213 52.6816L23.055 62.937L23.0213 73.1881C22.9875 76.6066 22.9749 80.0249 22.9158 83.4434C22.8343 90.2805 22.6698 97.1176 22.4224 103.955C22.1779 97.1176 22.0134 90.2805 21.9291 83.4434C21.87 80.0249 21.8616 76.6066 21.8237 73.1881L21.7899 62.9418L21.8279 52.6862C21.8616 49.2676 21.8742 45.8493 21.9333 42.4308C22.0148 35.5999 22.1779 28.7628 22.4224 21.9196Z",
  "M13.2967 31.426C13.5413 37.471 13.7057 43.516 13.7901 49.5606C13.8491 52.5831 13.8618 55.6056 13.8955 58.6281L13.9292 67.6955L13.9166 76.7627C13.8828 79.7852 13.8702 82.8077 13.8111 85.8302C13.7324 91.8749 13.568 97.9198 13.3178 103.965C13.0732 97.9198 12.9087 91.8749 12.8244 85.8302C12.7654 82.8077 12.7569 79.7852 12.719 76.7627L12.6515 67.6955L12.6852 58.6281C12.719 55.6056 12.7316 52.5831 12.7906 49.5606C12.8862 43.516 13.0549 37.471 13.2967 31.426Z",
  "M26.9811 44.2271C27.2425 49.2015 27.3777 54.1808 27.4702 59.16C27.5293 61.6498 27.542 64.1393 27.5757 66.629L27.6137 74.0932L27.58 81.562C27.542 84.0515 27.5336 86.5413 27.4745 89.0262C27.3817 94.0055 27.2425 98.9847 26.9811 103.964C26.7154 98.9847 26.5595 94.0055 26.4877 89.0262C26.4286 86.5413 26.4162 84.0515 26.3822 81.562L26.3485 74.0932L26.3822 66.629C26.4162 64.1393 26.4286 61.6498 26.4877 59.16C26.5595 54.1808 26.7113 49.2015 26.9811 44.2271Z",
  "M8.74083 49.2698C9.00227 53.8294 9.14144 58.3891 9.23421 62.944C9.28903 65.2259 9.30589 67.5033 9.33964 69.7808L9.37336 76.6179L9.33964 83.4549C9.30589 85.7372 9.29324 88.0146 9.23421 90.2968C9.14144 94.8517 9.00227 99.4111 8.74083 103.971C8.47937 99.4111 8.34022 94.8517 8.24745 90.2968C8.1884 88.0146 8.17577 85.7372 8.14202 83.4549L8.10828 76.6179L8.14202 69.7808C8.17998 67.5033 8.19263 65.2259 8.24745 62.944C8.34022 58.3891 8.47937 53.8294 8.74083 49.2698Z",
  "M4.17928 69.0691C4.44074 71.9785 4.57568 74.8875 4.66846 77.7921C4.72748 79.2445 4.74014 80.7015 4.77388 82.1539L4.81181 86.5154L4.77809 90.8769C4.74014 92.3293 4.73169 93.7862 4.67267 95.2384C4.57989 98.1429 4.44074 101.052 4.17928 103.962C3.91361 101.052 3.77868 98.1429 3.6859 95.2384C3.62687 93.7862 3.61422 92.3293 3.58048 90.8769L3.54675 86.5154L3.58048 82.1539C3.61422 80.7015 3.62687 79.2445 3.6859 77.7921C3.79554 74.8924 3.91782 71.983 4.17928 69.0691Z",
  "M31.5424 57.2831C31.804 61.1731 31.943 65.0631 32.0358 68.958C32.0908 70.9006 32.1076 72.848 32.1413 74.7954L32.175 80.6328L32.1413 86.4703C32.1076 88.4129 32.0949 90.3603 32.0358 92.3077C31.943 96.1977 31.804 100.088 31.5424 103.983C31.281 100.088 31.1207 96.1977 31.0492 92.3077C30.9902 90.3603 30.9775 88.4129 30.9438 86.4703L30.91 80.6328L30.9438 74.7954C30.9815 72.848 30.9942 70.9006 30.9492 68.958C31.1418 65.049 31.281 61.1589 31.5424 57.2831Z",
  "M36.0988 74.181C36.3602 76.6614 36.4994 79.1416 36.5922 81.6265C36.651 82.8666 36.6637 84.1067 36.6974 85.3467L36.7311 89.0717L36.6974 92.7968C36.6637 94.0368 36.651 95.2769 36.5922 96.517C36.5034 99.0019 36.3642 101.482 36.0988 103.967C35.8373 101.482 35.6769 99.0019 35.6054 96.517C35.5463 95.2769 35.5379 94.0368 35.4999 92.7968L35.4705 89.0717L35.5042 85.3467C35.5379 84.1067 35.5506 82.8666 35.6094 81.6265C35.7022 79.1416 35.8457 76.6614 36.0988 74.181Z",
];

const BASE_PATH =
  "M0 108.676C3.45336 107.373 7.0062 106.426 10.614 105.847C12.4104 105.522 14.2237 105.314 16.037 105.126C16.9437 105.022 17.8545 104.975 18.7654 104.932C19.6762 104.89 20.5829 104.81 21.4979 104.81H24.2348C25.1455 104.838 26.0606 104.866 26.9716 104.928C27.427 104.961 27.8824 104.98 28.3377 105.027L29.6998 105.173C30.6108 105.276 31.5215 105.357 32.4239 105.531C33.3266 105.706 34.2289 105.847 35.127 106.031C35.5783 106.116 36.0253 106.234 36.468 106.352L37.8048 106.705C38.6904 106.96 39.5761 107.177 40.4361 107.54L41.735 108.035L43 108.558L42.9622 108.719C39.4519 107.815 35.89 107.185 32.3017 106.833C28.7104 106.495 25.1054 106.369 21.5022 106.455C17.9009 106.515 14.3053 106.741 10.7152 107.134C7.13083 107.535 3.54645 108.078 0.0126503 108.827L0.0126503 108.676Z";

const STAR_PATH =
  "M26.9785 40.1048C26.9959 40.1035 27.0129 40.1096 27.0263 40.1219C27.0395 40.1342 27.0481 40.1516 27.0502 40.1708C27.0926 40.331 27.1263 40.4915 27.1767 40.6423C27.2306 40.8306 27.3265 41.0001 27.4558 41.1347C27.585 41.2692 27.7431 41.3644 27.9148 41.4109L28.2647 41.524C28.3111 41.524 28.3322 41.5666 28.3322 41.6091C28.3322 41.6513 28.3111 41.6749 28.2647 41.689C28.1255 41.7364 27.9823 41.7741 27.8431 41.826C27.6749 41.8872 27.5235 41.995 27.4025 42.1391C27.2817 42.2835 27.1954 42.4595 27.1514 42.651L27.0502 43.047C27.0502 43.0941 27.0124 43.1225 26.9785 43.1225C26.9447 43.1225 26.9196 43.0941 26.907 43.047C26.8606 42.8772 26.8268 42.6981 26.7678 42.5331C26.7092 42.3552 26.613 42.1958 26.4875 42.0685C26.3619 41.9415 26.2107 41.85 26.0467 41.8022L25.6925 41.689C25.6461 41.689 25.625 41.6468 25.625 41.6091C25.6248 41.5888 25.631 41.569 25.6425 41.5535C25.6542 41.5379 25.6705 41.5275 25.6881 41.524C25.84 41.477 25.9919 41.4393 26.1395 41.378C26.3026 41.3138 26.4492 41.2053 26.5654 41.0619C26.6817 40.9188 26.7644 40.7453 26.8058 40.5576L26.907 40.166C26.9108 40.1481 26.9199 40.1323 26.933 40.1214C26.9462 40.1101 26.9622 40.1043 26.9785 40.1048Z";

const GOLD = "#C19B4D";
const DRAW_MS = 2000;
const STAGGER_MS = 0;
const FILL_DELAY = DRAW_MS;
const WORDMARK_DELAY = FILL_DELAY + 100;
const HOLD_MS = 1100;
const FADE_MS = 500;
const TOTAL_MS = WORDMARK_DELAY + 200 + HOLD_MS + FADE_MS;

export const PRELOADER_DONE_EVENT = "golden:preloader-done";

export default function HomePreloader() {
  const [phase, setPhase] = useState<"drawing" | "filled" | "fading" | "done">(
    "drawing",
  );
  const [skip, setSkip] = useState(false);

  useEffect(() => {
    if (typeof window === "undefined") return;
    const links: HTMLLinkElement[] = PRELOAD_IMAGES.map((href) => {
      const link = document.createElement("link");
      link.rel = "preload";
      link.as = "image";
      link.href = href;
      link.fetchPriority = "high";
      document.head.appendChild(link);
      return link;
    });
    const cleanupLinks = () => {
      links.forEach((l) => l.parentNode?.removeChild(l));
    };

    if (sessionStorage.getItem("golden-preloader-shown") === "1") {
      setSkip(true);
      setPhase("done");
      window.dispatchEvent(new Event(PRELOADER_DONE_EVENT));
      return cleanupLinks;
    }

    const t1 = window.setTimeout(() => setPhase("filled"), FILL_DELAY);
    const t2 = window.setTimeout(
      () => setPhase("fading"),
      WORDMARK_DELAY + 200 + HOLD_MS,
    );
    const t3 = window.setTimeout(() => {
      setPhase("done");
      sessionStorage.setItem("golden-preloader-shown", "1");
      window.dispatchEvent(new Event(PRELOADER_DONE_EVENT));
    }, TOTAL_MS);
    return () => {
      window.clearTimeout(t1);
      window.clearTimeout(t2);
      window.clearTimeout(t3);
      cleanupLinks();
    };
  }, []);

  if (skip || phase === "done") return null;

  const filled = phase === "filled" || phase === "fading";

  return (
    <div
      aria-hidden
      className="pointer-events-none fixed inset-0 z-[200] flex flex-col items-center justify-center bg-black"
      style={{
        opacity: phase === "fading" ? 0 : 1,
        transition: `opacity ${FADE_MS}ms cubic-bezier(0.32, 0.72, 0, 1)`,
      }}
    >
      <div className="relative mx-auto flex h-[55vh] max-h-[460px] min-h-[280px] w-full max-w-[360px] flex-col items-center justify-center">
        <svg
          viewBox="0 0 43 109"
          fill="none"
          aria-hidden
          className="h-full"
          style={{ aspectRatio: "43 / 109" }}
        >
          {ICON_PATHS.map((d, i) => (
            <path
              key={i}
              d={d}
              fill={filled ? GOLD : "none"}
              stroke={GOLD}
              strokeWidth={filled ? 0 : 0.4}
              pathLength={1}
              style={{
                strokeDasharray: 1,
                strokeDashoffset: filled ? 0 : 1,
                animation: filled
                  ? undefined
                  : `golden-preloader-draw ${DRAW_MS}ms cubic-bezier(0.32, 0.72, 0, 1) ${i * STAGGER_MS}ms forwards`,
                transition: filled
                  ? `fill 400ms ease, stroke-width 400ms ease`
                  : undefined,
              }}
            />
          ))}
          <path
            d={BASE_PATH}
            fill={filled ? GOLD : "none"}
            stroke={GOLD}
            strokeWidth={filled ? 0 : 0.4}
            pathLength={1}
            style={{
              strokeDasharray: 1,
              strokeDashoffset: filled ? 0 : 1,
              animation: filled
                ? undefined
                : `golden-preloader-draw ${DRAW_MS}ms cubic-bezier(0.32, 0.72, 0, 1) ${ICON_PATHS.length * 60}ms forwards`,
              transition: filled
                ? `fill 400ms ease, stroke-width 400ms ease`
                : undefined,
            }}
          />
          <path
            d={STAR_PATH}
            fill="url(#preloaderStar)"
            style={{
              opacity: filled ? 1 : 0,
              transition: `opacity 400ms ease`,
            }}
          />
          <defs>
            <linearGradient
              id="preloaderStar"
              x1="26.9786"
              y1="40.1046"
              x2="26.9786"
              y2="43.1225"
              gradientUnits="userSpaceOnUse"
            >
              <stop stopColor="#C19B4D" />
              <stop offset="1" stopColor="#9C7013" />
            </linearGradient>
          </defs>
        </svg>

        <div
          className="mt-6 flex w-full items-center justify-center"
          style={{
            opacity: filled ? 1 : 0,
            transform: filled ? "translateY(0)" : "translateY(8px)",
            transition: `opacity 400ms ease, transform 400ms ease`,
          }}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/golden-center.svg"
            alt="Golden Group"
            className="mx-auto block h-auto w-[200px] md:w-[240px]"
          />
        </div>
      </div>

      <style>{`
        @keyframes golden-preloader-draw {
          from { stroke-dashoffset: 1; }
          to { stroke-dashoffset: 0; }
        }
      `}</style>
    </div>
  );
}
